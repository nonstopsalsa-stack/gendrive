$edge = Start-Process -FilePath "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" -ArgumentList "--headless", "--remote-debugging-port=9223", "--disable-gpu" -PassThru
Start-Sleep -Seconds 2

try {
  $fileUrl = "file:///" + ((Get-Item .\tests\test_mobile_sync_and_navigation.html).FullName.Replace('\', '/'))
  $page = Invoke-RestMethod -Method Put -Uri "http://localhost:9223/json/new?$fileUrl"
  Start-Sleep -Seconds 2
  ...
  Write-Host "=================================================="
  Write-Host " Gendrive Mobile v1.5.9 Test Suite Results"
  Write-Host "=================================================="
